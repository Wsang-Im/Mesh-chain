 ...
 *
 * RSU provides the following functions as roadside infrastructure:
 * - V2I communication (WAVE IEEE 802.11p, 300m range)
 * - L1 Anchoring (Local, per-RSU, 30-60 second period)
 * - L2 Anchoring (Regional, multi-RSU, 60-180 second period)
 * - L3 Anchoring (Global, cloud, 300-600 second period)
 * - Block relay and storage
 * - Certificate issuance (ML-DSA-65 signature)
 *
 * Deployment strategy:
 * - Highway: 400m spacing (considering DSRC 300m range)
 * - Coverage: Overlapping regions secured (STR ≥ 90%)
 * - Cost: $20k-52k/km for 1km spacing (we use denser deployment)
 */
...
struct RSUConfig {
    ...
    // Physical location (SUMO coordinate system)
    double position_x;  // meters
    double position_y;  // meters

    // Communication range (DSRC standard)
    double communication_range_m;  // default 300m

    // Anchoring periods
    uint32_t l1_anchor_period_sec;  // 30-60 seconds (Local)
    uint32_t l2_anchor_period_sec;  // 60-180 seconds (Regional)
    uint32_t l3_anchor_period_sec;  // 300-600 seconds (Global)

    // Block storage capacity
    size_t max_blocks_stored;  // default 10000 blocks

    // Cloud connection availability
    bool has_cloud_connection;

    // L1 anchor file export directory
    std::string l1_export_dir;  // default: "/tmp/meshchain_l1"
};
...
struct AnchorBlock {
    ...
    std::vector<std::string> participating_rsus;  // L2/L3 only
    std::vector<std::vector<uint8_t>> multi_sigs;  // L2/L3 multi-signatures

    // Previous anchor reference (chain structure)
...
public:
    ...
    /**
     * Start RSU (background anchoring thread)
     */
    void start() {
        if (running_) return;
    ...
    /**
     * Receive block from vehicle (V2I)
     */
    ...
    /**
     * Calculate distance to RSU (based on vehicle position)
     */
    ...
    /**
     * Query anchor block (vehicle checks if its block is anchored)
     */
    ...
    /**
     * Check if specific block is anchored
     */
    ...
    /**
     * Check if specific vehicle's blocks are anchored
     *
    ...
    std::tuple<bool, Hash256, size_t> isVehicleChainAnchored(const std::string& vehicle_id) const {
        ...
        // Check the most recent L1 anchor
        if (l1_anchors_.empty()) {
            return {false, Hash256{}, 0};
        }

        // Check how many blocks from this vehicle are included in anchored_blocks_
        ...
        // Merkle root of the most recent anchor
     ...
     *
     * @param vehicle_id Vehicle ID
     * @param anchor_index Anchor index (default: most recent anchor)
     * @return Merkle proof (vehicle chain root → top-level anchor root)
     */
    std::optional<std::vector<Hash256>> getVehicleChainProof(
        ...
        // Currently just returns anchor root
...
private:
    ...
    /**
     * L1 Anchoring loop (runs every 30-60 seconds)
     */
    void l1AnchorLoop() {
...
public:
     ...
     * Create L1 Anchor
     *
     * Paper Algorithm 3: RSU Anchoring
     *
    ...
    void createL1Anchor() {
        ...
        while (!pending_anchor_blocks_.empty()) {
            ...
            // Only anchor locally-final blocks (exclude TENTATIVE)
        ...
        // Step 6: RSU signature (sigA ← Dilithium.Sign)
    ...
    /**
     * Block validation (including TEE commit validation)
     */
    bool validateBlock(const Block& block) const {
        ...
        // Check locally-final state
        if (block.header.state != BlockState::LOCALLY_FINAL) {
            return false;  // Don't anchor TENTATIVE blocks
        }

        // NEW: Attestquorum protocol - Check witness participation
        if (block.header.witness_bitmap.count() == 0) {
            return false;  // No witnesses participated
        }

        // Attestquorum validation (simplified - RSU trusts blocks with valid attestquorum)
    ...
    /**
     * Export L1 anchor to JSON file (for L2 aggregation)
     */
