...
#ifdef USE_TRACI
        try {
            // GUI 모드: 별도 프로세스로 sumo-gui 실행 (libsumo GUI 메모리 버그 회피)
            if (config_.use_gui) {
                ...
                if (ret == 0) {
                    ...
                    std::this_thread::sleep_for(std::chrono::seconds(2));  // GUI가 시작될 시간
                } else {
                    std::cerr << "[TraCI] Warning: Failed to launch SUMO GUI, continuing headless\n";
                }
            }

            // libsumo는 항상 headless로 실행 (GUI 크래시 회피)
...
private:
    ...
    void updateMockVehicleStates() {
        ...
        const int num_vehicles = 50;  // 20 → 50 (더 많은 차량으로 ToF 검증 성공률 향상)
        const double highway_length = 1000.0; // 2km → 1km (더 밀집된 환경)
