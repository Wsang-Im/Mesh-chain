...
public:
    struct Config {
        ...
        // If communication records exist, create block immediately
        // Local finality ≤100ms is block creation SPEED
...
public:
    explicit IntegratedVehicle(const Config& config)
        ...
        libp2p_->subscribe("meshchain/blocks",
            [this](const LibP2PNode::Message& msg) {
                ...
                // if (valid) {
                //     std::cout << "[" << vehicle_id_ << "] ✓ Block accepted\n";
                // }
            }
        );
        ...
        // Range: 0.5 ensures ≥0.3 difference is achievable
    ...
    void step(const std::map<std::string, std::shared_ptr<IntegratedVehicle>>& all_vehicles) {
        ...
        // 4. SIMPLE: Create block immediately if V2X communication records exist
        // CRITICAL: Prevent duplicate block creation (prevents memory corruption)
        if (has_v2x_data_.load() && !is_creating_block_.load()) {
            ...
            if (is_creating_block_.compare_exchange_strong(expected, true)) {
                std::cout << "[" << vehicle_id_ << "] 📦 V2X data received → Creating block\n";
    ...
    void recordWitnessObservation(const Block& block) {
        ...
        // Extract witnesses who signed from block header (attestquorum protocol)
        // Use witness_bitmap to determine which witnesses participated
        for (size_t i = 0; i < block.header.witness_bitmap.size(); ++i) {
            if (block.header.witness_bitmap.test(i)) {
    ...
    double calculateWitnessWeight(double reputation) const {
        ...
        // This gives:
        // R=0.3 → w≈0.05 (very low weight)
        // R=0.5 → w=0.5  (neutral)
        // R=0.7 → w≈0.95 (high weight)
        ...
        const VehicleID& block_creator_id)
    {
        ...
        if (is_malicious) {
            std::cout << "  ⚠️  MALICIOUS BLOCK DETECTED!\n";
            ...
            if (!valid_reports.empty()) {
                ...
                if (!silent_witnesses.empty()) {
                    std::cout << "  ⚠️  Found " << silent_witnesses.size()
    ...
    void printMicroChainSummary() const {
        ...
        if (!my_micro_chain_.empty()) {
            ...
            for (size_t i = start; i < my_micro_chain_.size(); i++) {
                ...
                std::cout << "  Block #" << i << ": "
                          << "witnesses=" << block.header.witness_bitmap.count()
...
private:
    ...
    void setupMessageHandlers() {
        ...
        wave_->registerCallback(WaveStack::MessageType::CAM,
            [this](const WaveStack::WaveMessage& msg) {
                ...
                if (!msg.cam.has_value()) {
                    std::cout << "[" << vehicle_id_ << "] ⚠ WARNING: CAM callback invoked but msg.cam is empty!\n";
                    return;
                }
                ...
                // NEW: Set compute_vote_mac function for attestquorum protocol
                // In production, this would send a request to the witness over TLS 1.3
                // For simulation, we compute MAC locally using HMAC-SHA256
                candidate.compute_vote_mac = [witness_id = cam.sender_id](
                    const Hash256& block_hash,
                    const VehicleID& creator_id) -> std::vector<uint8_t> {

                    // Simulate MAC computation: HMAC-SHA256(block_hash, witness_secret)
                    // In production: MAC is computed by witness and returned over TLS channel
                    std::vector<uint8_t> mac(32);  // HMAC-SHA256 = 32 bytes

                    // Use witness_id as "secret key" for simulation
                    std::vector<uint8_t> key_material(witness_id.begin(), witness_id.end());

                    // Simple MAC simulation: XOR block_hash with hashed key
                    Hash256 key_hash = crypto::SHA3::hash(key_material);

                    for (size_t i = 0; i < 32; ++i) {
                        mac[i] = block_hash[i] ^ key_hash[i];
                    }
                ...
                // V2X communication record received → Set block creation flag (atomic)
        ...
        wave_->registerCallback(WaveStack::MessageType::DENM,
            [this](const WaveStack::WaveMessage& msg) {
                ...
                std::cout << "[" << vehicle_id_ << "] 🚨 DENM received from "
    ...
    void tryCreateBlock(const std::map<std::string, std::shared_ptr<IntegratedVehicle>>& all_vehicles) {
        ...
        if (candidates.size() < MIN_WITNESS_COUNT) {
            std::cout << "[" << vehicle_id_ << "] ⚠ Insufficient witnesses (" << candidates.size()
                ...
                if (block_opt.has_value()) {
                    {
                        ...
                        std::cout << "[" << vehicle_id_ << "] ✓ Block added to micro-chain (total: "
    ...
    void broadcastBlock(const Block& block) {
        ...
        // Log TEE attestquorum information (NEW PROTOCOL)
        if (block.header.use_attestquorum) {
            std::cout << "[" << vehicle_id_ << "]   TEE attestquorum: "
                      << block.header.attestquorum.size() << " bytes (ECDSA P-256)\n";
            std::cout << "[" << vehicle_id_ << "]   Witness bitmap: "
                      << block.header.witness_bitmap.count() << " witnesses\n";
        } else {
            std::cout << "[" << vehicle_id_ << "]   No attestquorum (legacy fallback)\n";
        }
        ...
        std::cout << "[" << vehicle_id_ << "] ✓ Block broadcasted via WAVE + libp2p\n";

        // NOTE: Do NOT send to RSU directly!
        // RSU retrieves blocks from this vehicle's micro-chain via pull-based method
        // through onAnchorRequest() at L1 anchoring time (every 60 seconds).
    }
    ...
    bool verifyReceivedBlock(const Block& block) {
        ...
        if (!block.header.validateStructure()) {
            std::cout << "[" << vehicle_id_ << "] ✗ Block structure invalid\n";
            return false;
        }
        ...
        if (block.header.creator_sig.empty()) {
            std::cout << "[" << vehicle_id_ << "] ✗ Missing creator signature\n";
            return false;
        }

        // Step 3: TEE attestquorum verification (NEW PROTOCOL)
        if (block.header.use_attestquorum) {
            std::cout << "[" << vehicle_id_ << "]   Verifying TEE attestquorum (ECDSA P-256)...\n";

            // Verify attestquorum signature
            // In production: verifyECDSA(attestquorum, Hash(Header)||bitmap, TEE_pubkey)
            // For simulation: check signature is present and non-empty
            if (block.header.attestquorum.empty()) {
                std::cout << "[" << vehicle_id_ << "] ✗ Attestquorum signature missing\n";
                return false;
            }

            // Check witness bitmap
            if (block.header.witness_bitmap.count() == 0) {
                std::cout << "[" << vehicle_id_ << "] ✗ No witnesses in bitmap\n";
                return false;
            }

            std::cout << "[" << vehicle_id_ << "] ✓ Attestquorum verified ("
                      << block.header.witness_bitmap.count() << " witnesses, "
                      << block.header.attestquorum.size() << " bytes)\n";
            return true;
        }

        // Step 4: Legacy fallback (should not happen with new protocol)
        std::cout << "[" << vehicle_id_ << "]   ⚠ No attestquorum - legacy mode not supported\n";
        return false;
    }
