 ...
 *
 * 공격 대응 능력을 체계적으로 측정하고 분석하는 프레임워크
 *
 * 주요 기능:
 * 1. 각 방어 메커니즘의 효과성 측정
 * 2. 공격 탐지율 및 차단율 계산
...
enum class DefenseMechanism {
    SIGNATURE_VERIFICATION,   // PQC 서명 검증
    REPUTATION_SCREENING,     // 평판 기반 스크리닝
    DIVERSITY_VALIDATION,     // 증인 다양성 검증
    TOF_PROXIMITY_CHECK,      // ToF 거리 측정 검증
    RATE_LIMITING,            // 속도 제한
    WITNESS_CONSENSUS,        // 증인 합의 검증
    TEE_ATTESTATION,          // TEE 증명 검증
    MERKLE_PROOF,             // Merkle 증명 검증
};
...
struct DefenseStats {
    ...
    size_t false_positives = 0;    // 정상을 공격으로 오인
    size_t false_negatives = 0;    // 공격을 정상으로 오인
