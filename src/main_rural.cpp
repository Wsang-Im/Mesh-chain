...
public:
    ...
    bool initialize() {
        ...
        std::cout << "✓ GUI: " << (config_.sumo_use_gui ? "Enabled" : "Disabled") << "\n\n";
    ...
    void initializeRSUs() {
        ...
        std::vector<RSUPosition> positions = {
            ...
            // 24 RSUs deployed for complete coverage (4 rows × 6 columns)
            // 300m communication range, ~1000m spacing between RSUs
            ...
            // Row C (y=1000) - South-Center
            {"RSU-7-C0",  1000.0, 0.0},
            {"RSU-8-C1",  1000.0, 500.0},
            {"RSU-9-C2",  1000.0, 1000.0},
            {"RSU-10-C3", 1000.0, 1500.0},
            {"RSU-11-C4", 1000.0, 2000.0},
            {"RSU-12-C5", 1000.0, 2500.0},

            // Row E (y=2000) - North-Center
            {"RSU-13-E0", 2000.0, 0.0},
            {"RSU-14-E1", 2000.0, 500.0},
            {"RSU-15-E2", 2000.0, 1000.0},
            {"RSU-16-E3", 2000.0, 1500.0},
            {"RSU-17-E4", 2000.0, 2000.0},
            {"RSU-18-E5", 2000.0, 2500.0},

            // Row G (y=3000) - Far North
            {"RSU-19-G0", 3000.0, 0.0},
            {"RSU-20-G1", 3000.0, 500.0},
            {"RSU-21-G2", 3000.0, 1000.0},
            {"RSU-22-G3", 3000.0, 1500.0},
            {"RSU-23-G4", 3000.0, 2000.0},
            {"RSU-24-G5", 3000.0, 2500.0},
        };
        ...
        std::cout << "  - Grid Layout: 4×6 RSU deployment pattern\n";
        std::cout << "  - Row A (y=0m): 6 RSUs [A0-A5]\n";
        std::cout << "  - Row C (y=1000m): 6 RSUs [C0-C5]\n";
        std::cout << "  - Row E (y=2000m): 6 RSUs [E0-E5]\n";
        std::cout << "  - Row G (y=3000m): 6 RSUs [G0-G5]\n";
        std::cout << "  - Coverage: 3500m × 2500m (complete grid network)\n";
        std::cout << "  - RSU Spacing: ~1000m (optimal for 300m range)\n";
        std::cout << "  - Range: " << range_m << "m (300m V2I communication)\n";
...
private:
    ...
    void updateVehicles() {
        ...
        for (const auto& [vid, state] : vehicle_states) {
            ...
            config.wave_config.enable_realistic_mac = true;  // MAC layer enabled
            ...
            // SIMPLE: Generate block immediately upon receiving V2X communication records (no config needed)
...
int main(int argc, char** argv) {
    std::cout << "==================================================\n";
    std::cout << "  Mesh-Chain Texas Rural Town Scenario\n";
